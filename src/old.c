...
int main(void)
{
	...
	float fTemperature = 0; //otrzymana temperatura, float
	char sTemperature[7];
	...
	_delay_ms(1000);		 //odczekanie 10000ms
	...
	while (1) //petla glowna programu
	{
		...
		if (ucReset == 1)
		{
			...
			dtostrf(fTemperature, 2, 4, sTemperature); // Konwertowanie floata do stringa

			LcdClear();			   //wyczyszczenie wyswietlacza
			LcdBin(cTemperatureH); //wyswietla MSB
			LcdBin(cTemperatureL); //wyswietla LSB
			LcdGoto(0, 1);		   //przejscie do kolejnej linii
			LcdWrite(sTemperature);
			LcdWrite(" ");
			LcdDecComma((fTemperature * 10), 1); //wyswietlenie temperatury
		}
		else
		{
			LcdClear();			 //wyczyszczenie wyswietlacza
			Lcd("BRAK ODCZYTU"); //wyswietlenie napisu
		}

		 _delay_ms(2000); //odczekanie 1000ms
	}
}